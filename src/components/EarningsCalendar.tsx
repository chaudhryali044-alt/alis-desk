...
function EarningsRow({ item }: { item: EarningsItem }) {
  ...
  return (
    <a
      href={`https://finance.yahoo.com/quote/${encodeURIComponent(item.symbol)}/financials`}
      target="_blank"
      ...
      >
        ...
        {item.reportDateFmt && (
          <div
            ...
            style={{ color: 'var(--text-muted)', minWidth: 38, textAlign: 'center', lineHeight: 1.25 }}
          >
            {item.reportDateFmt}
          </div>
        )}
...
export default function EarningsCalendar() {
  ...
  useEffect(() => {
    fetchEarnings();
    const t = setInterval(fetchEarnings, 15 * 60_000);
  ...
  return (
    <div className="card" style={{ padding: '16px 16px 12px' }}>
      ...
      {loading ? (
        ...
        >
          Earnings data unavailable — retrying shortly
        </div>
      ) : (
        <div>
          {items.map(item => (
